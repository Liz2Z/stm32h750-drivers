...
| -------------- | ----------- | ------------ | --------------------------------------------------------- |
| PB0            | GPIO_Output | BLK          | 背光控制 (高电平点亮) 点亮背光 ≠ 显示白色                 |
| PB1            | GPIO_Output | RS/D/C       | Data/Command 区分数据(1)和命令(0)信号                     |
| PB12           | GPIO_Output | CS           | 片选信号，低电平选中屏幕                                  |
| PB13           | GPIO_Output | SCK/SCL      | 时钟线，同步数据传输                                      |
| PB14           | GPIO_Input  | MISO/SDO     | Master In Slave Out 主入从出，STM32读取屏幕数据（如读状态 |
| PB15           | GPIO_Output | MOSI/SDI     | Master Out Slave In 主出从入，STM32向屏幕发送命令/数据    |

工作原理：

拉低 CS 选中屏幕
通过 D/C 告诉屏幕接下来是命令还是数据
在 SCK 时钟驱动下，通过 MOSI 发送数据
BLK 控制背光开关
